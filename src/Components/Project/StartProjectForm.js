...
const StartProjectForm = ({ handleWarningDiscard, edit, projectdata }) => {
  ...
  var unique = [...defaultConnections, ...myConnections].filter(onlyUnique); // returns ['a', 1, 2, '1']
  ...
  function submit() {
    fetch(kBaseUrl + "update_project", {
      ...
      body: JSON.stringify({
        ...
        project_members: teamMembers.length !== 0 ? teamMembers : tms,
      }),
    })
      .then((res) => res.json())
      .then((data) => {
        ...
        prj = myprojects.filter((prjc) => prjc._id === data._id);
        const emprj = myprojects.filter((prjc) => prjc._id !== data._id);
        setMyProjects([prj, ...emprj]);
        setTeamMembers([]);
        setTechnologies([]);
        setRequirements([]);
